...
def test_cbs_table49_branch_paths(monkeypatch) -> None:
    ...
    monkeypatch.setattr(collector, "_download_excel", lambda: b"excel")
    monkeypatch.setattr(collector, "_parse_excel", lambda _content: (_ for _ in ()).throw(ValueError("bad")))
    assert list(collector.collect()) == []
    ...
    with pytest.raises(ValueError):
        _latest_price_column(pd.DataFrame([[None], [None], [None], [None], [None], [None], [None], [None]]))
